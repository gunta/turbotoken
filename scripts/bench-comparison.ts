...
const tiktokenCheck = runShell(
  "python3 -c \"import importlib.util,sys;sys.exit(0 if importlib.util.find_spec('tiktoken') else 1)\"",
  { allowFailure: true },
);
...
if (tiktokenCheck.code === 0) {
  commands.push({
    ...
    command:
      "python3 -c \"import pathlib,tiktoken;text=pathlib.Path('bench/fixtures/english-100kb.txt').read_text();tiktoken.get_encoding('o200k_base').encode(text)\"",
  });
} else {
