...
Benchmark-note: Added dedicated native C ABI byte-path benchmark (`bun run bench:native-bytes`) to track ARM64 NEON encode/decode kernels independently from scalar BPE; latest NEON vs scalar artifact is `bench/results/bench-native-byte-path-20260227-145853.json` (encode: `81.0 ms` vs `109.9 ms`, ~1.36x faster; decode: `80.5 ms` vs `116.7 ms`, ~1.45x faster).
Benchmark-note: Added native pretokenizer kernel benchmark (`bun run bench:native-pretokenizer`) with runtime auto-selection over NEON/DotProd variants; latest artifact is `bench/results/bench-native-pretokenizer-20260227-145747.json` (unicode auto: `100.0 ms`, scalar: `169.3 ms`, ~1.69x faster; auto selected kernel id `1` = NEON on this M4 Max build).
Training-note: Added first-pass BPE training APIs (`train_mergeable_ranks_from_iterator`, `train_encoding_from_iterator`) plus `scripts/bench-training.ts`, and wired an experimental Zig-native training backend (`TURBOTOKEN_TRAINING_BACKEND=native`) through C ABI. Latest baseline artifact (`bench/results/bench-training-python-20260227-145645.json`) now puts turbotoken Python fallback (`49.05 ms`) and native (`49.39 ms`) ahead of `rustbpe` (`55.74 ms`) and `minbpe` (`703.18 ms`) on the tracked 100KB fixture; native ASCII O200K pretokenize/direct paths (including multi-text direct C ABI) remain opt-in experiments (`TURBOTOKEN_TRAIN_NATIVE_PRETOKENIZE=1`, `TURBOTOKEN_TRAIN_NATIVE_DIRECT_ASCII=1`).
Benchmark-note: Added canonical scorecard aggregation (`bun run bench:scorecard`) that emits a single benchmark summary artifact (`bench/results/bench-scorecard-1772209744278.json`) plus Markdown snapshot (`bench/charts/scorecard.md`) from the latest measured startup/competitor/training/RSS/GPU-memory rows.
Optimization-note: Core encode/count BPE native calls now route through reusable rank-bound native sessions (`NativeRankSession`), and rank payload loading now prefers compiled native blobs (`*.tiktoken.native.bin`) by default; default decode path remains Python-first while native decode is kept opt-in (`TURBOTOKEN_NATIVE_DECODE_ENABLE=1`) after regression checks.
...
| 5.3 | Benchmarks on RTX 4090 / A100 | `TODO` | CUDA memory benchmark harness is in-tree (`scripts/bench-gpu-memory-cuda.ts`) and emits per-process/allocator memory + MiB/s rows when run on NVIDIA hosts; added Modal remote runner (`scripts/modal/bench_cuda_modal.py`) to execute startup/RAM/competitor/training + CUDA memory rows on Modal NVIDIA GPUs (maps `RTX4090` alias to `L40S`). Latest Modal B200 run (`bench/results/bench-modal-cuda-1772191604329.json`) used `nvidia/cuda:13.1.1-cudnn-runtime-ubuntu24.04`; full tokenizer throughput runs on dedicated RTX/A100 hardware still pending |
...
| 2026-02-25 | Keep `hybrid` rank-BPE queue mode as default; expose `full-bucket` mode for experiments *(superseded on 2026-02-27)* | Switch default queue mode to `full-bucket` immediately | Queue A/B remained mixed/noisy (`bench/results/bench-encoder-queue-20260225-180932.json`, `bench/results/bench-encoder-queue-20260225-181051.json`) |
| 2026-02-27 | Switch rank-BPE queue default to `full-bucket`; keep `hybrid` override | Keep `hybrid` default | Refreshed queue A/B and post-switch scalar fallback checks favored/validated `full-bucket` for encode path (`bench/results/bench-encoder-queue-20260227-145729.json`, `bench/results/bench-scalar-fallback-20260227-145659.json`) |
| 2026-02-27 | Precompute single-byte token rank map in rank loader and route merged-node init through direct byte-rank lookup | Keep per-byte hash lookups in `buildMergedNodes` | Focused scalar fallback rerun improved count and slightly improved encode without correctness regressions (`bench/results/bench-scalar-fallback-20260227-142803.json`) |
