...
| 2026-02-25 | Pair-cache slot hash default switched to `rapidhash`; keep ARM64 `crc32` as opt-in env mode | Keep previous default hash, switch to `crc32` default | New A/B (`bench/results/bench-pair-cache-hash-20260225-182315.json`) kept `rapidhash` as cross-platform default while preserving `crc32` for ARM64-specific checks |
