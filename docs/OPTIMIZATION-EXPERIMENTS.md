...
Evaluate pair-cache slot hash choices (`rapidhash`, `wyhash`, ARM64 `crc32`) and promote a better default only with measured wins.
...
- Added runtime hash selector in `src/pair_cache.zig`:
  ...
  - `TURBOTOKEN_PAIR_CACHE_HASH=wyhash` (legacy baseline)
...
| Command | Mean |
|---|---:|
| `rapidhash-count-bpe-100kb` | `1.115 s` |
| `wyhash-count-bpe-100kb` | `1.131 s` |
| `crc32-count-bpe-100kb` | `1.115 s` |
| `rapidhash-encode-bpe-100kb` | `1.455 s` |
| `wyhash-encode-bpe-100kb` | `1.496 s` |
| `crc32-encode-bpe-100kb` | `1.442 s` |

Decision: `adopt` (`rapidhash` default) and `keep optional` (`crc32`, `wyhash`).
Reason: latest run shows `rapidhash` faster than `wyhash` on both count and encode; `crc32` remains mixed/noisy.
